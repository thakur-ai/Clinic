...
const AdminContactsPage = () => {
  ...
  return (
    <>
      <div className="min-h-screen bg-gradient-to-br from-slate-50 to-slate-100 p-2 sm:p-6 lg:p-10 font-sans">
        {" "}
        {/* Enhanced background, reduced padding for mobile */}
        <div className="max-w-7xl mx-auto bg-white rounded-3xl shadow-2xl shadow-slate-300/70 overflow-hidden border border-white/80">
          {" "}
          {/* Stronger shadow and rounded corners */}
          {/* Header Section */}
          <div className="p-5 sm:p-8 border-b border-slate-100 flex flex-col md:flex-row items-start md:items-center justify-between gap-4 bg-white/95 backdrop-blur-sm sticky top-0 z-10 shadow-sm sm:shadow-none">
            {" "}
            {/* Added sticky header */}
            <div>
              ...
              <p className="text-slate-500 text-sm mt-1">
                Manage incoming inquiries from your patients and website
                visitors.
              </p>{" "}
            ...
            <div className="flex flex-col sm:flex-row w-full md:w-auto items-center gap-3">
              {" "}
              {/* Responsive filter/refresh group */}
          ...
          {/* Content Area */}
          <div className="bg-slate-50/30 min-h-[400px] p-2 sm:p-6 lg:p-8">
            {" "}
            {/* Added responsive padding, reduced padding for mobile */}
            {loading && (
              <div className="flex flex-col items-center justify-center py-20 bg-blue-50 rounded-lg shadow-inner border border-blue-100">
                {" "}
                {/* Blue themed loading */}
            ...
            {error && (
              <div className="p-8 text-center bg-red-50 rounded-lg border-t-2 border-red-200 shadow-inner">
                {" "}
                ...
                {/* More descriptive error title */}
                <p className="text-red-600 mt-2 mb-6 text-base">{error}</p>
                <button
                  onClick={fetchContacts}
                  className="px-5 py-2.5 bg-white text-red-600 font-semibold rounded-lg shadow-sm hover:bg-red-50 transition-colors border border-red-200"
                >
                  Try Again
                </button>
              </div>
            ...
            {!loading && !error && filteredContacts.length === 0 && (
              <div className="flex flex-col items-center justify-center py-20 text-center px-4 bg-slate-50 rounded-lg shadow-inner border border-slate-100">
                {" "}
                ...
                {/* Bolder text */}
                <p className="text-slate-600 max-w-sm mx-auto text-base">
                  {" "}
                  {/* Wider max-width for text */}
                  There are no {filter !== "All"
                    ? filter.toLowerCase()
                    : ""}{" "}
                  inquiries to display right now. Check back later!
                </p>
              </div>
            )}
            {!loading && !error && filteredContacts.length > 0 && (
              <>
                        ...
                        >
                          <td className="px-6 py-4 whitespace-nowrap">
                            <div className="flex items-center">
                              <div className="h-9 w-9 rounded-full bg-slate-100 flex items-center justify-center mr-3 text-slate-500 font-bold text-sm shadow-inner">
                                {" "}
                                {/* Slightly larger avatar, added shadow */}
                          ...
                          <td className="px-6 py-4 whitespace-nowrap">
                            <span
                              className={`px-3 py-1 inline-flex text-sm leading-5 font-bold rounded-full border ${getBadgeStyle(
                                contact.inquiryType
                              )}`}
                            >
                              {" "}
                              {/* Larger badge text/padding */}
                              {contact.inquiryType}
                            </span>
                          </td>
                ...
                {/* --- MOBILE VIEW (Cards) --- */}
                <div className="md:hidden flex flex-col gap-0.5 p-0.5">
                  {" "}
                  {/* Further reduced gap and padding for cards */}
                  {filteredContacts.map((contact) => (
                    <div
                      key={contact._id}
                    ...
                    >
                      ...
                      <div className="flex justify-between items-center mb-0 pb-0 border-b border-slate-100">
                        {" "}
                        {/* Further reduced mb and pb */}
                        <div className="flex items-center gap-0.5">
                          {" "}
                          {/* Reduced gap */}
                          <div className="h-6 w-6 rounded-full bg-gradient-to-br from-blue-100 to-blue-200 flex items-center justify-center text-blue-700 font-bold shadow-sm text-xs">
                            {" "}
                            {/* Reduced avatar size */}
                            {contact.name.charAt(0)}
                          </div>
                          <div>
                            <h3 className="font-semibold text-slate-900 text-[0.7rem] leading-tight">
                              {contact.name}
                            </h3>{" "}
                            {/* Further reduced name font size */}
                            <div className="flex items-center gap-0.5 mt-0">
                              <span
                                className={`px-0.5 py-0 text-[0.6rem] font-semibold uppercase tracking-tight rounded-full border ${getBadgeStyle(
                                  contact.inquiryType
                                )}`}
                              >
                                {contact.inquiryType}
                        ...
                        <span className="text-[0.65rem] font-medium text-slate-500 bg-slate-50 px-0.5 py-0 rounded-md shadow-sm">
                          {" "}
                          {/* Reduced date badge padding */}
                          {format(new Date(contact.date), "MMM dd")}
                        </span>
                      </div>
                      {/* Card Body: Message */}
                      {/* The message will be shown in the modal on mobile screens */}
                      <div className="hidden md:block bg-slate-50 rounded-md p-1.5 mb-1.5 border border-slate-100 text-center">
                        {" "}
                        {/* Reduced padding and margin */}
                        <MessageSquare className="w-3 h-3 text-slate-400 mx-auto mb-1" />{" "}
                        {/* Reduced message icon size and margin */}
                        <p className="text-xs text-slate-700 leading-relaxed italic">
                          {" "}
                          {/* Reduced message font size */}"{contact.message}"
                        </p>
                      </div>
                      {/* Card Footer: Actions */}
                      <div className="flex flex-col sm:flex-row items-center justify-between pt-0.5 border-t border-slate-100 gap-0.5">
                        {" "}
                        {/* Further reduced pt and gap */}
                        <a
                          href={`mailto:${contact.email}`}
